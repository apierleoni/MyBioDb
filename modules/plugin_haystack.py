...
class WhooshBackend(SimpleBackend):
    ...
    def indexes(self,*fieldnames):
        ...
        self.fieldnames = fieldnames
        self.indexname =".".join([self.table._db._uri_hash, self.table._tablename, 'whoosh'])
        try:
        ...
        with self.ix.searcher() as searcher:
            for fieldname in fieldkeys:
                ...
                if mode == 'and':
                    ...
                    ids = new_ids if ids is None else ids | new_ids
        return list(ids)
    def simple_search(self, query_str, limit):
        from whoosh.qparser import MultifieldParser
        ids = None
        with self.ix.searcher() as searcher:

            parser = MultifieldParser(self.fieldnames, schema=self.ix.schema)
            query = parser.parse(unicode(query_str))
            results = searcher.search(query,limit=limit)
            ids = set(long(result['id']) for result in results)

        return list(ids)
    def rebuild(self):
        db = self.table._db
        writer = self.ix.writer()
        for row in db(db[self.table].id >0).select():
            try:
                writer.update_document(id=unicode(row.id),
                                       **dict((name,unicode(row[name]))
                                              for name in self.fieldnames if name in row))
            except:
                if DEBUG: print "error building index for table ", self.table._tablename, " id: ",row.id
    ...
    def indexes(self,*fieldnames):
        ...
        if not os.path.exists(self.schema_filename):
            schema='<fields><field name="id" type="int" indexed="true" stored="true" required="true" />%s</fields>' \
                % ''.join('<field name="%s" type="string" />' % name for name in fieldnames)
            open(self.schema_filename,'w').write(schema)
        try:
...
class Haystack(object):
    ...
    def simple_search(self, query, limit = 20):
        ids = self.backend.simple_search(query, limit)
        return self.table._id.belongs(ids)
    def rebuild(self):
        self.check_fields()
        self.backend.rebuild()
    def check_fields(self, *fieldnames):
